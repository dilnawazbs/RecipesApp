...
public class RecipeControllerTest {
    ...
    void shouldCreateRecipe() throws Exception {
        Recipe recipe = new Recipe("1","Fried egg with tomato", 2, TestUtil.getIngredients("potato"), "crack the egg on the pan with little oil.", Category.NON_VEGETARIAN );
    ...
    void shouldReturnRecipe() throws Exception {
        String id = "1";
        Recipe recipe = new Recipe(id, "Fried egg with tomato", 2, TestUtil.getIngredients("potato", "cauliflower"), "crack the egg on the pan with little oil.", Category.NON_VEGETARIAN );
    ...
    void shouldReturnListOfRecipes() throws Exception {
        ...
        List<Recipe> recipes = new ArrayList<>(
            Arrays.asList(new Recipe("0", "Fried egg with tomato", 2, TestUtil.getIngredients("egg", "tomato"), "crack the egg on the pan with little oil.", Category.NON_VEGETARIAN),
    ...
    void shouldReturnListOfRecipeWithFilter_Vegetarian() throws Exception {
        List<Recipe> recipes = new ArrayList<>(Arrays.asList(new Recipe("1", "Kadai paneer", 3, TestUtil.getIngredients("paneer", "bell pepper"), "firstly, in a large kadai, heat 1 tbsp butter and saute 1 bay leaf, 1 green chilli, 1 tsp kasuri methi till they aromatic.", Category.VEGETARIAN),
                new Recipe("2", "mixed veg", 2, TestUtil.getIngredients("bell pepper", "potato", "mushroom"), "chop all the ingredients and stir it in kadai with oil and spices", Category.VEGETARIAN)
            ));
    ...
    void shouldUpdateRecipe() throws Exception {
        ...
        Recipe existingRecipe = new Recipe(id, "Fried egg with tomato", 2, TestUtil.getIngredients("egg", "tomato"), "crack the egg on the pan with little oil.", Category.NON_VEGETARIAN);
        Recipe updatedRecipe = new Recipe(id, "Fried egg with tomato", 4, TestUtil.getIngredients("egg", "tomato"), "crack the egg on the pan with little oil.", Category.NON_VEGETARIAN);
        ...
        mockMvc.perform(put("/recipes/{id}", id).contentType(MediaType.APPLICATION_JSON)
    ...
    void shouldMergePatchUpdateRecipe() throws Exception {
        ...
        mockMvc.perform(patch("/recipes/{id}", id).contentType(PatchMediaType.APPLICATION_MERGE_PATCH)
    ...
    void shouldPatchUpdateRecipe() throws Exception {
        ...
        mockMvc.perform(patch("/recipes/{id}", id).contentType(PatchMediaType.APPLICATION_JSON_PATCH)
    ...
    void shouldPatchUpdateRecipe_UnSupportedMediaType() throws Exception {
        ...
        mockMvc.perform(patch("/recipes/{id}", id).contentType(MediaType.APPLICATION_JSON)
                .content(TestUtil.fromFile("json-patch.json")))
